        ...
        for field in &record {        
            ...
            if field!="" && contador==1 //NÚMERO DE DOCUMENTO DE IDENTIFICACIÓN
            {
                ...
                {//RUC     
                    ...
                    else if es_ruc(field)==true
                    {
                        //valido=1;
                    }
                    let stringaux1= field.chars().nth(10).unwrap().to_string();
                    let aux1 : i32 = stringaux1.parse().unwrap();
                    let stringaux2= field.chars().nth(11).unwrap().to_string();
                    let aux2 : i32 = stringaux2.parse().unwrap();
                    let stringaux3= field.chars().nth(12).unwrap().to_string();
                    let aux3 : i32 = stringaux3.parse().unwrap();
                    if aux1>=0 && aux2>=0 && aux3>=1
                    {
